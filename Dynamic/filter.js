const show_filter = (cluster,spec) => {
  if(cluster == "ring"){redraw_rings(spec)}
  change_view("filter")
  ...
  filter_parts.sort().map((part,partIndex) => {
      ...
      .on("mouseover",() => {transition_highlight(`${spec_wsc}.${part_wsc}`)})
      .on("mouseout",() => {delight()})
      .on("click",()=> {show_points(cluster,spec,part)})
  })
}
