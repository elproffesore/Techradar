  ...
  if(searchword == ""){
   ...
   .style('transform','rotate(-90deg)')
    ...
    rings.sort().map((ring,ringIndex) => {
      ...
      .on("mouseover",() => {array.map(p => d3.select("#"+p.id).attr("r",points_radius*1.5))})
      .on("mouseout",() => {d3.selectAll(".circles").attr("r",points_radius)})
      .on("click",() => {show_filter_search(results_array,ring,searchword)})
...
const show_filter_search = (search_array,ring,searchword) => {
  change_view("filter")
  ...
  topics.sort().map((topic,topicIndex) => {
    ...
    .on("mouseover",() => {array.map(p => {single_highlight(p.id)})})
    .on("mouseout",() => {delight()})
    .on("click",()=> {show_points_search(search_array,ring,topic,searchword,0)})
  })
}
const show_points_search = (search_array,ring,topic,searchword,page) => {
  change_view("points")
  ...
  d3.select("#points > .header > img").attr("src","Data/Pics/arrow_left.svg")
   .on("click",() => {show_filter_search(search_array,ring,searchword)})
